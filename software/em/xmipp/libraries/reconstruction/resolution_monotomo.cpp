...
void ProgMonoTomoRes::produceSideInfo()
{
	...
	transformer_inv.FourierTransform(inputVol, fftV);
	iu.initZeros(fftV);
	...
	transformer_inv.FourierTransform(V1(), *fftN);
...
void ProgMonoTomoRes::firstMonoResEstimation(MultidimArray< std::complex<double> > &myfftV,
...
void ProgMonoTomoRes::medianFilter3x3x3(const MultidimArray<double> &inputVol, MultidimArray<double> &filteredVol)
{
	std::cout << "Filtering ..." << std::endl;

	FOR_ALL_ELEMENTS_IN_ARRAY3D(inputVol)
	{
		A3D_ELEM(inputVol, k, i, j) = 1;
		A3D_ELEM(inputVol, k, i, j) = 1;
		A3D_ELEM(inputVol, k, i, j) = 1;
		A3D_ELEM(inputVol, k, i, j) = 1;
		A3D_ELEM(inputVol, k, i, j) = 1;
...
void ProgMonoTomoRes::resolution2eval(int &count_res, double step,
...
//			std::cout << "z = " << z << "  zcritical = " << criticalZ << std::endl;
	...
	postProcessingLocalResolutions(pOutputResolution, list, cut_value, pMask);
